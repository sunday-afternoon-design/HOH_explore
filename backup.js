...
document.addEventListener("DOMContentLoaded", function() {
    ...
    function init() {
        for (let i = 0; i < imgcnt; i++) {
            ...
            imgArray1.push(mesh1)
            imgArray2.push(mesh2)
            imgArray3.push(mesh3)
            imgArray4.push(mesh4)
    ...
    function animate() {
        ...
        for (let i = 0; i < 9; i++) {
            if (imgArray[0].position.z < 0.05) {
                let ZmovingSpeed = .05 / 24 / 10;

                imgArray[i].position.z += ZmovingSpeed;
                imgArray1[i].position.x += -.05 / 24 / 10
                imgArray1[i].position.y += -.03 / 24 / 10
                imgScale += .06 / 24 / 10;
                imgArray[i]
                    .scale
                    .set(imgScale, imgScale, imgScale);
                ...
                imgArray1[i].position.x = -.5 + -.05 * i
                imgArray1[i].position.y = -.3 + -.03 * i
                imgScale = .06 * i + 1
                imgArray[i]
                    .scale
                    .set(imgScale, imgScale, imgScale);
    ...
    class marchingImg {
        constructor(material, imgRatio, z) {
            this.mesh = new THREE.Mesh(new THREE.PlaneGeometry(.5 * imgRatio, .5), material);
        }
    }
